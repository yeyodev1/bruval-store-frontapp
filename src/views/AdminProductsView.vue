...
const confirmation = computed(() => ({ created: 'Producto creado correctamente.', updated: 'Cambios guardados correctamente.', deleted: 'Producto eliminado del catálogo.' }[String(route.query.status)] || ''))
...
<template>
  <main class="catalog-admin">
    ...
    <section v-else class="product-grid" aria-label="Productos"><RouterLink v-for="product in filteredProducts" :key="product._id" class="product-card" :to="`/admin/productos/${product._id}`"><img :src="product.image" :alt="product.name"><span><b>{{ product.name }}</b><small>{{ product.sku }} · {{ product.collection }}</small><strong><del v-if="product.regularPrice && product.regularPrice > product.price" style="opacity: 0.6; margin-right: 4px; font-weight: normal; text-decoration: line-through;">{{ formatPrice(product.regularPrice) }}</del>{{ formatPrice(product.price) }}</strong></span><em>Editar</em></RouterLink><p v-if="!filteredProducts.length" class="status">No encontramos productos con esa búsqueda.</p></section>
  </main>
</template>
...
.success { margin:16px 0 0; padding:12px 14px; color:#386341; background:#e2f0e5; border:1px solid #b9d9bf; border-radius:5px; font-size:13px; }
</style>
